const homeLeaderBoardSQLQuery = `select th.team_name as name,
  ...
  from matches as m join teams as th on th.id = m.home_team where in_progress = false 
  group by home_team order by totalPoints desc, totalVictories desc, goalsBalance desc,
goalsFavor desc, goalsOwn asc;`;

export default homeLeaderBoardSQLQuery;
